...
fit.ns_r6 <- function(points, lims, R, disp = "gaussian", child.dist = "pois", child.info = NULL,
                      edge.correction = "pbc", start = NULL, siblings = NULL, trace = FALSE){
    ...
    } else if (child.dist == "twoplane"){
        use.twoplanechild.class <- TRUE
        child.list <- list(twoplane.w = child.info$w,
                           twoplane.b = child.info$b,
                           twoplane.l = child.info$l,
                           twoplane.tau = child.info$tau)
    } else {
